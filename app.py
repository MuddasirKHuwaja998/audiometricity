...
def disegna_audiogramma(
    ...
    plt.figure(figsize=(10, 5))  # Reduced height from 6 to 5
...
def generate_pdf(image_path, paziente_info):
    ...
    info_table = Table(info_table_data, colWidths=[190, 310], hAlign='CENTER')  # Adjusted colWidths to move a bit more to the right
    info_table.setStyle(TableStyle([
        ('FONT', (0, 0), (-1, -1), 'Times-Roman', 10),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
        ('LEFTPADDING', (0, 0), (-1, -1), 60),  # Adjusted padding to move a bit more to the right
    ...
    # Audiogram image
    elements.append(Image(image_path, width=450, height=250))  # Reduced height from 300 to 250
    elements.append(Spacer(1, 12))
    ...
    # Footer
    firma_style = ParagraphStyle(name='Firma', fontName='Times-Roman', fontSize=10, alignment=2)  # Alignment to right
    ...
    elements.append(Spacer(1, 12))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.black))  # Added horizontal line at the end
